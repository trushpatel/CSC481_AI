...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghost in self.allPositions:
            self.beliefs[ghost] *= self.getObservationProb(observation, pac, ghost, jail)
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for pos1 in self.allPositions:
            newDist = self.getPositionDistribution(gameState, pos1)
            for pos2 in newDist:
                new[pos2] += newDist[pos2] * self.beliefs[pos1]
        new.normalize()
        self.beliefs = new
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        pac = gameState.getPacmanPosition()
        jail = self.getJailPosition()
        pred = self.getBeliefDistribution()
        for ghost in self.allPositions:
            pred[ghost] *= self.getObservationProb(observation, pac, ghost, jail)
        if pred.total() == 0:
            self.initializeUniformly(gameState)
            return
        pred.normalize()
        self.particles = [pred.sample() for _ in range(self.numParticles)]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for s in ghosts:
            for i in range(self.numGhosts):
                ...
                obs = observation[i]
                ghost = s[i]
                pred[s] *= self.getObservationProb(obs, pac, ghost, jail)
        pred.normalize()
        if pred.total() == 0:
            self.initializeUniformly(gameState)
        else:
